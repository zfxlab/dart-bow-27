...
- generate_config.cmake：辅助函数集中在文件开头；统一浮点、整数、布尔参数输出，合并重复默认值块；用一个本地输出函数替代写文件、读回、替换占位符、再次写文件的流程。内容未变化时保留输出时间戳。
- 根 CMakeLists.txt：电机与遥控器的重复路径筛选改成直接循环；补齐 app 头文件路径收集；OpenOCD 配置与源文件清单改为内容变化时更新。
...
当前两板构建机制能够继续使用，无需重写为另一套构建框架。父 CMake 的职责基本合理；主要维护压力集中在较长的 generate_config.cmake，以及生成器和 GUI 校验之间需要同步的规则。
...
合并顺序为公共默认值、板型默认值、用户 params，按字段递归覆盖；数组整体替换。缺少一个字段不会丢失同模块的其他默认常量。公共缺省关闭 USBX、板载消费者、遥控器和测试；板型 profile 可以显式覆盖，例如当前 F407 profile 启用 BMI088、USBX 和 USB 诊断。串口推荐值不等于已启用的绑定。
...
继续收敛根 CMake 的源码选择，并按完整职责整理生成器。无需把生成器拆成大量小文件；优先减少重复规则、隐含作用域和跨文件同步修改。涉及公共 BSP 或生成规则的改动必须复用双板矩阵，并把构建结果与硬件验证分开记录。
